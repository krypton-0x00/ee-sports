...
import { DeleteAccountSchema, forgetPasswordSchema, idSchema, LoginSchema, RegisterSchema, resetPasswordSchema, VerifyEmailSchema } from "../validation/auth.validation";
import { z } from 'zod'
...
export default class AuthService {
    ...
    async forgetPassword(req: Request) {
        try {
            ...
            await this.authRepo.updateUserById(user.id, { resetPasswordToken: resetToken, resetPasswordExpiry: new Date(tokenExpiry) })
    ...
    async resetPassword(req: Request) {
        try {
            ...
            if (!user) {
                return createResponse(400, false, "User not found or the token has been expired .")
            }
            const hashedPassword: string = await bcrypt.hash(password, 10);

            await this.authRepo.updateUserById(user.id, { passwordHash: hashedPassword, resetPasswordToken: null, resetPasswordExpiry: null })
